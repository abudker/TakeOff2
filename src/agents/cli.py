...
def check_claude_cli():
    ...
    if not shutil.which("claude"):
        raise click.ClickException(
            ...
            "The extraction system requires Claude Code to invoke agent workers.\n"
            "Please install Claude Code: https://claude.ai/download"
        )


def count_extracted_items(building_spec: Dict[str, Any]) -> Dict[str, int]:
    """Count extracted items from a building spec dict."""
    return {
...
def extract_one(eval_id: str, evals_dir: Path, output: Path, verbose: bool):
    ...
    if not eval_dir.exists():
        raise click.ClickException(f"Evaluation case not found: {eval_dir}")
    ...
    if final_state.get("error"):
        raise click.ClickException(final_state["error"])

    # Get building spec
    building_spec = final_state.get("building_spec")
    if not building_spec:
        raise click.ClickException("No building spec in final state")
    ...
    click.echo(f"  Components: {counts['zones']} zones, {counts['walls']} walls, {counts['windows']} windows, {counts['hvac']} HVAC, {counts['dhw']} DHW")
...
def extract_all(evals_dir: Path, skip_existing: bool, force: bool, verbose: bool):
    ...
    if not manifest_path.exists():
        raise click.ClickException(f"manifest.yaml not found in {evals_dir}")
