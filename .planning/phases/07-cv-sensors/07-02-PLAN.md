...
must_haves:
  truths:
    - "CV sensor measurements appear in orientation pass prompts when running canterbury-rd eval"
    - "Orientation extraction on canterbury-rd produces a result (no errors) with CV hints injected"
    - "Running with --no-cv flag produces results without CV measurements (backwards compatible)"
    - "Instruction files contain 'Using CV Hints' sections and 'Fallback' labels for non-CV paths"
  artifacts:
  ...
  key_links:
    ...
    - from: "pass1-north-arrow.md"
      ...
      pattern: "CV_HINTS"
    - from: "test_orientation_twopass.py"
      to: "src/agents/orchestrator.py"
      via: "from agents.orchestrator import run_cv_sensors"
      pattern: "from agents\\.orchestrator import.*run_cv_sensors"
---
...
2. Add new MODULE-LEVEL PUBLIC function `run_cv_sensors(eval_dir: Path, document_map: DocumentMap) -> dict` (must be importable as `from agents.orchestrator import run_cv_sensors`):
...
1. Update `pass1-north-arrow.md`:
   ...
   SIMPLIFY the existing visual estimation instructions by making specific removals:

   **Remove from Step 1:**
   - Delete the "Which side of vertical does it lean?" disambiguation block (lines ~46-50 in current file: the bullet list with "Leans LEFT", "Leans RIGHT", "Straight UP"). Replace with: "Fallback (no CV hints): Estimate visually — see the direction table in Step 3."
   - Delete "If arrow looks nearly vertical" sentence (line ~50) — CV measurement handles this.

   **Remove from Step 3:**
   - Delete the 8-row direction table (lines ~68-78: "TOP of page | 0°" through "Upper-left | 315°"). Replace with: "Fallback (no CV hints): Estimate the direction the front faces on the page. Use cardinal directions: 0°=top, 90°=right, 180°=bottom, 270°=left. Interpolate for diagonals."
   - This single sentence replaces 10 lines of table, offsetting the CV hint additions.

   The net result: CV hint sections add ~15 lines, removals cut ~15 lines. File stays approximately the same length (85-95 lines).
...
2. Update `pass2-elevation-matching.md`:
   ...
   SIMPLIFY the "Measure the entry direction" subsection in Step 2 with specific removals:

   **Remove from Step 2:**
   - Delete the "Measure the entry direction" subsection body (lines ~73-89 in current file): the numbered steps "1. Trace the wall edge line..." and the 5-line "Examples" block. Replace with: "Fallback (no CV hints): Trace the entry wall edge, measure its angle from horizontal, then determine the perpendicular outward direction. See direction reference: 0°=top, 90°=right, 180°=bottom, 270°=left."

   **Remove from Step 3:**
   - Delete the 3-bullet tilt disambiguation block (lines ~96-99: "Arrow tip leans LEFT", "Arrow tip leans RIGHT", "Arrow points straight UP"). Replace with: "Fallback (no CV hints): Estimate visually — left-of-vertical = 360° - tilt, right = tilt, vertical = 0°."

   The net result: CV hint sections add ~15 lines, removals cut ~15 lines. File stays approximately the same length (110-125 lines).
...
3. KEY DESIGN PRINCIPLE: The LLM still does ALL semantic reasoning:
  ...
  <verify>
Run structural checks on both instruction files:

```bash
cd /Users/Andrew/Projects/takeoff2

# 1. Check line counts (should be 85-130 lines each, not bloated)
wc -l .claude/instructions/orientation-extractor/pass1-north-arrow.md .claude/instructions/orientation-extractor/pass2-elevation-matching.md

# 2. Check for "Using CV Hints" sections in BOTH files
grep -c "Using CV Hints" .claude/instructions/orientation-extractor/pass1-north-arrow.md
grep -c "Using CV Hints" .claude/instructions/orientation-extractor/pass2-elevation-matching.md
# Expected: >= 1 occurrence in each file

# 3. Check for "Fallback" labels in BOTH files
grep -c "Fallback" .claude/instructions/orientation-extractor/pass1-north-arrow.md
grep -c "Fallback" .claude/instructions/orientation-extractor/pass2-elevation-matching.md
# Expected: >= 1 occurrence in each file

# 4. Verify the old verbose blocks were removed from Pass 1
grep -c "Leans LEFT" .claude/instructions/orientation-extractor/pass1-north-arrow.md
# Expected: 0 (removed)

# 5. Verify the old verbose blocks were removed from Pass 2
grep -c "Arrow tip leans LEFT" .claude/instructions/orientation-extractor/pass2-elevation-matching.md
# Expected: 0 (removed)
```

Verify: Both files have "Using CV Hints" sections (>= 1 match each), "Fallback" labels (>= 1 match each), removed disambiguation blocks (0 matches for old verbose content), and line counts in 85-130 range.
  </verify>
  <done>
...
1. Update `test_orientation_twopass.py`:
   - Import with: `from agents.orchestrator import run_cv_sensors` (this works because Task 1 creates `run_cv_sensors` as a module-level function in orchestrator.py, not a nested/internal function)
