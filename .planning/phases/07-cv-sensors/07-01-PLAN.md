...
<task type="auto">
  <name>Task 2a: North arrow detection</name>
  <files>src/cv_sensors/north_arrow.py</files>
  <action>
Create `src/cv_sensors/north_arrow.py` with:
...
assert result['confidence'] != 'none', 'Should detect north arrow on canterbury-rd'
assert 0 <= result['angle'] < 360, 'Angle in valid range'
"`
Verify: North arrow detection returns a dict with angle (should be near 0 for canterbury-rd), confidence is not "none".
  </verify>
  <done>
    - `detect_north_arrow_angle` detects arrow and returns compass bearing
    - Returns structured dict with angle, confidence, method, debug fields
    - Coordinate system conversions handle OpenCV's inverted y-axis correctly
  </done>
</task>

<task type="auto">
  <name>Task 2b: Wall edge detection and building rotation</name>
  <files>src/cv_sensors/wall_detection.py</files>
  <action>
Create `src/cv_sensors/wall_detection.py` with two functions:

1. `measure_wall_edge_angles(pdf_path: str, page_num: int, building_region: tuple = None, zoom: float = 2.0) -> list`
...
2. `estimate_building_rotation(pdf_path: str, page_num: int, building_region: tuple = None, zoom: float = 2.0) -> dict`
...
Run: `cd /Users/Andrew/Projects/takeoff2 && python3 -c "
import sys; sys.path.insert(0, 'src')
from cv_sensors.wall_detection import measure_wall_edge_angles, estimate_building_rotation
walls = measure_wall_edge_angles('evals/canterbury-rd/plans.pdf', 3)
print(f'Wall edges ({len(walls)}): {walls[:3]}')
assert len(walls) > 0, 'Should detect wall edges'
assert all(0 <= w['angle_from_horizontal'] < 180 for w in walls), 'Angles in valid range'
rot = estimate_building_rotation('evals/canterbury-rd/plans.pdf', 3)
print(f'Building rotation: {rot}')
"`
Verify: Wall detection returns a non-empty list with angle and length fields. Building rotation returns structured dict.
  </verify>
  <done>
