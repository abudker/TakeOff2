...
**First, verify prerequisite CLIs exist:**
```bash
python -m agents --help && python -m verifier --help
```
...
```bash
# First verify prerequisite CLIs exist
cd /Users/Andrew/Projects/takeoff2 && python -m agents --help && python -m verifier --help && echo "Prerequisite CLIs exist"

# Then verify improvement CLI
...
1. Prerequisite CLIs exist: `python -m agents --help` and `python -m verifier --help` succeed
2. `python -m improvement improve --help` shows command options
3. `python -m improvement rollback --help` shows command options
4. improve command loads eval results and computes aggregate metrics
5. improve command would invoke critic (actual invocation tested in Plan 04)
6. rollback command finds and restores instruction snapshots
</verification>

<success_criteria>
- Prerequisite CLIs (agents, verifier) are accessible before integration
