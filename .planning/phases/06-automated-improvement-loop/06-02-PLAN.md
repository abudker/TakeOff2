...
must_haves:
  truths:
    ...
    - "Loop handles errors gracefully (extraction/critic/apply failures) and can resume"
  artifacts:
...
def loop(evals_dir, max_iterations, target_f1, patience, min_improvement, resume_from, no_commit):
    ...
    Error handling: If extraction, critic, or apply fails, the loop logs the error,
    increments the failure counter, and continues to the next iteration. After 3
    consecutive failures, the loop stops. Successful iterations reset the counter.
...
- KeyboardInterrupt: Save current state to history, display partial progress, exit cleanly
  </action>
  <verify>
```bash
...
```
  ...
  <done>loop command runs automated improvement loop with configurable target F1, max iterations, patience, and resume capability; handles errors gracefully with consecutive failure tracking</done>
</task>
...
- loop command handles errors gracefully (logs, continues, stops after 3 consecutive failures)
