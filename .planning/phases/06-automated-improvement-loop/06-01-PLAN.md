...
must_haves:
  truths:
    ...
    - "Each iteration is committed with metrics (enables rollback via git)"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/improvement/loop.py"
      to: "src/improvement/cli.py"
      via: "git_commit_iteration for persisting changes with metrics"
      pattern: "from .cli import.*git_commit_iteration"
---
...
**Complexity note:** This task is intentionally larger because it's the orchestration layer that ties everything together. The complexity is inherent to the orchestration role - splitting would create artificial boundaries.
...
class ImprovementLoop:
    def __init__(
        ...
        auto_commit: bool = True  # Whether to call git_commit_iteration
    )
    ...
    def run_single_iteration(self) -> tuple[bool, dict]:
        ...
        IMPORTANT: Must call git_commit_iteration(proposal, before_metrics, after_metrics, iteration, project_root)
        when auto_commit=True and iteration succeeds. This ensures each iteration is committed with metrics,
        enabling rollback via git (requirement IMP-06).
        """
...
- On success with auto_commit=True: call git_commit_iteration to persist with metrics
...
**Error handling:**
- On extraction failure: increment consecutive_failures, log error, continue to next iteration
- On critic failure: increment consecutive_failures, log error, continue
- On apply failure: increment consecutive_failures, log error, continue
...
# Verify git_commit_iteration is imported and callable
python3 -c "
from src.improvement.loop import ImprovementLoop
import inspect
source = inspect.getsource(ImprovementLoop.run_single_iteration)
assert 'git_commit_iteration' in source, 'run_single_iteration must call git_commit_iteration'
print('git_commit_iteration integration verified')
"
```
  </verify>
  <done>ImprovementLoop class orchestrates automated iterations with stop conditions (target F1, plateau, max iterations), calls git_commit_iteration for each successful iteration (enabling rollback), handles errors gracefully with consecutive failure tracking, displays progress via Rich</done>
</task>
...
- [ ] run_single_iteration calls git_commit_iteration (verified via source inspection)
...
- Loop calls git_commit_iteration after each successful iteration (IMP-06: commits with metrics enable rollback)
